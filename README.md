...
│   │   ├── wrist.csv
│   │   └── labels.csv
│   └── eval/                 # eval datasets (ppg2026_dataset.npz etc.)
...
### Multi-dataset training (March + ppg2026)

All training scripts support comma-separated `--data` paths to combine multiple NPZ files.
Data are concatenated, then split together (train/val/test). HR is re-zscored on the combined set.

```bash
# SBP regression (PPG + HR)
python src/train_march_sbp_torch.py \
  --data march_sbp_dataset.npz,data/eval/ppg2026_dataset.npz \
  --save-dir results/combined_sbp --plot

# SBP regression (PPG only)
python src/train_march_sbp_ppg_only_torch.py \
  --data march_sbp_dataset.npz,data/eval/ppg2026_dataset.npz \
  --save-dir results/combined_sbp_ppg_only --plot

# State classification (binary: rest vs wallsit compatible)
python src/train_march_state_torch.py \
  --data march_sbp_dataset.npz,data/eval/ppg2026_dataset.npz \
  --mode binary --save-dir results/combined_state --plot
```
...
### PPG branch mode (`--ppg-mode`)

All training scripts support `--ppg-mode`:

- **full** (default): use both finger and wrist PPG.
- **finger**: use finger PPG only.
- **wrist**: use wrist PPG only.

Example:

```bash
python src/train_march_sbp_torch.py --data march_sbp_dataset.npz --ppg-mode finger --save-dir results/sbp_finger --plot
python src/train_march_sbp_torch.py --data march_sbp_dataset.npz --ppg-mode wrist --save-dir results/sbp_wrist --plot
```
...
PPG-only scripts also support `--ppg-mode full|finger|wrist`.

### Evaluate trained SBP model

Load weights and evaluate on any March-format NPZ:
...
(generated by `align_ppg_cuff.py`), you can convert it to the same NPZ format as March:

```bash
python src/eval/preprocess_ppg2026_to_march.py --ppg2026-dir /path/to/ppg2026
```

Output: `data/eval/ppg2026_dataset.npz` (default). Then evaluate:

```bash
python src/eval/eval_march_sbp.py --data data/eval/ppg2026_dataset.npz --ckpt results/all_train/best_state_dict.pt
``` with `X`, `hr`, `y`, `group`, `state`, etc., compatible with `train_march_*.py` and `eval/eval_march_sbp.py`.
