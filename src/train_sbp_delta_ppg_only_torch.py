"""
PPG only -> ΔSBP. 与 train_sbp_delta_torch 相同，但无 HR 输入。
"""
...
from train_march_sbp_ppg_only_torch import Model, TrainConfig, get_device, set_seed, shuffle_labels
...
def train_all(
    ...
    for epoch in range(cfg.epochs):
        ...
        for s in range(0, len(tr_idx), cfg.batch_size):
            ...
            pred = model(_to_tensor(Xtr[b], device))
        ...
        model.eval()
        with torch.no_grad():
            pv = model(_to_tensor(Xva, device)).cpu().numpy()
...
def main() -> None:
    p = argparse.ArgumentParser(description="Train ΔSBP model (PPG only, no HR)")
    ...
    print(f"Device: {device} ppg_mode={args.ppg_mode} [PPG only, ΔSBP target]")

    paths = [x.strip() for x in args.data.split(",") if x.strip()]
    if not paths:
        raise ValueError("--data required")
    X_list, y_list, group_list, state_list = [], [], [], []
    for path in paths:
        ...
        print(f"  Loaded {path}: X {z['X'].shape}, y {z['y'].shape}")
    ...
    if (~keep).sum() > 0:
        X = X[keep]
        print(f"ΔSBP: dropped {(~keep).sum()} samples (no rest baseline)")
    print(f"ΔSBP range: [{y.min():.1f}, {y.max():.1f}], mean={y.mean():.1f}, std={y.std():.1f}")

    if args.shuffle_labels:
        y = shuffle_labels(y, args.seed)

    out = train_all(X, y, cfg, device, float(args.val_ratio), args.ppg_mode)
    print("-" * 60)
    ...
    if args.save_dir:
        ...
        if args.plot and plt:
            ...
            ax.set_title("Train/Val loss (ΔSBP, PPG only)")
            ax.legend()
