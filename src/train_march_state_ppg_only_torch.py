"""
PPG only -> State classification (PyTorch, GPU-ready).

Same backbone as `train_march_state_torch.py` but without HR input:
- Dual PPG branches (finger + wrist) only.
- No hr_mlp; head input is d_model*2.
...
def train_with_indices(
    ...
    Xtr, Ytr = X[train_idx], y_cls[train_idx]
    Xva, Yva = X[val_idx], y_cls[val_idx]
    ...
    for epoch in range(cfg.epochs):
        ...
        for s in range(0, len(balanced_idx_all), cfg.batch_size):
            ...
            logits = model(_to_float(Xtr[b], device))
        ...
        model.eval()
        with torch.no_grad():
            logits_va = model(_to_float(Xva, device))
    ...
    model.eval()
    with torch.no_grad():
        logits_va = model(_to_float(Xva, device)).cpu().numpy()
...
def main() -> None:
    ...
    model.eval()
    with torch.no_grad():
        logits_te = model(_to_float(X[te_idx], device)).cpu().numpy()
    y_te = y_cls[te_idx]
    ...
    if args.save_dir:
        ...
        if args.plot and plt is not None:
            ...
            ax.set_ylabel("True")
            ax.set_title(f"Confusion matrix ({args.mode}, PPG only)")
