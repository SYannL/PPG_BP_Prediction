"""
PPG only -> SBP (PyTorch, GPU-ready).

Same architecture as train_march_sbp_torch.py but without HR input:
...
def train_all(
    ...
    for epoch in range(cfg.epochs):
        ...
        model.eval()
        with torch.no_grad():
            pv_t = model(_to_tensor(Xva, device))
...
def main() -> None:
    ...
    if args.save_dir:
        ...
        if args.plot and plt is not None:
            ...
            ax.set_title("Train/Val loss curves (PPG only)")
